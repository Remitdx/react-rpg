...
export function TeamArea({team, order, onClick, bossHealth, characterOneHealth, characterTwoHealth, characterThreeHealth}) {
  return <div className="fight-item team-area d-flex justify-content-around align-items-end">
    <div key={team[0].identity} className="d-flex flex-column">
      { characterOneHealth }
      {characterOneHealth == 0 ? <Character size="avatar-sm" dead={true} character={team[0]} /> : <Character size={order[0] == team[1] ? "avatar" : "avatar-sm"} character={team[0]} />}
    ...
    <div key={team[1].identity} className="d-flex flex-column">
      { characterTwoHealth }
      {characterTwoHealth == 0 ? <Character size="avatar-sm" dead={true} character={team[1]} /> : <Character size={order[0] == team[2] ? "avatar" : "avatar-sm"} character={team[1]} />}
    ...
    <div key={team[2].identity} className="d-flex flex-column">
      ...
      {characterThreeHealth == 0 ? <Character size="avatar-sm" dead={true} character={team[2]} /> : <Character size={order[0] == team[3] ? "avatar" : "avatar-sm"} character={team[2]} />}
